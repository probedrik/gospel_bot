...
async def start_command(message: Message, state: FSMContext, db=None):
    ...
    # Сохраняем пользователя в БД (гарантированно)
    try:
        # В aiogram 3.x диспетчер передает сохраненные данные как дополнительные аргументы
        if db:
            ok = await db.add_user(user_id, username, first_name)
            if not ok:
                logger.warning(
                    "Не удалось добавить пользователя в БД через /start")
    ...
    welcome_text = (
        ...
        "**🔥 Возможности:**\n"
        "• 📖 Книги и главы Библии\n"
        "• 🔍 Поиск ссылок — например: `Ин 3:16`\n"
        "• 🤖 ИИ-разбор стихов (3/день)\n"
        "• 📝 Закладки и заметки\n"
        "• 📚 Планы чтения\n"
        "• 🎯 Темы с ИИ\n"
        "• 📅 Православный календарь\n\n"
        "🚀 Начните с меню ниже или просто отправьте ссылку на стих."
    )
