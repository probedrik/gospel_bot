...
# Настройки квот
# Лимит для администраторов (практически без ограничений)
ADMIN_DAILY_LIMIT = 1000
...
class AIQuotaManager:
    ...
    async def _quota_reset_loop(self):
        ...
        while True:
            try:
                ...
                logger.info(
                    f"⏰ Следующий сброс квот: {next_reset.strftime('%Y-%m-%d %H:%M:%S')} UTC")
    ...
    async def get_user_quota_info(self, user_id: int) -> dict:
        ...
        try:
            ...
            logger.error(
                f"❌ Ошибка получения квоты для пользователя {user_id}: {e}")
        ...
        try:
            ...
            if premium_available > 0 or is_free_premium_user or is_admin_premium_mode:
                ...
                if quota_info['can_use_ai']:
                    ...
                    user_type = "админ" if user_id == ADMIN_USER_ID else (
                        "бесплатный премиум" if is_free_premium_user else "премиум")
                    ...
                    if premium_available > 0:
                        ...
                        logger.warning(
                            f"⚠️ Бесплатный премиум пользователь {user_id} исчерпал дневные лимиты")
                        return False, 'none'
            ...
            logger.error(
                f"❌ Ошибка проверки квоты для пользователя {user_id}: {e}")
            return False, 'none'
